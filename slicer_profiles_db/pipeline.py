...
class ProfilePipeline:
    ...
    def ingest(
        ...
        if evaluated_request:
            ...
            if evaluated:
                return evaluated
            raise DownloadError(
                f"No evaluated {slicer.value} profile bundle found for {version}"
            )
    ...
    def ingest_all_versions(
        ...
        if config.ini_bundle:
            reports = self.ingest_all_ini_versions(slicer, profile_types)
            if config.evaluated_profile_bundle_index:
                evaluated = self.ingest_evaluated_profile_bundle(slicer, profile_types)
                if evaluated:
    ...
    def ingest_evaluated_profile_bundle(
        self,
        slicer: SlicerType,
        profile_types: list[ProfileType] | None = None,
        ...
        config = get_source_config(slicer)
        index_url = config.evaluated_profile_bundle_index
        if not index_url:
            return None
        ...
        if requested_version and normalize_version(requested_version).startswith("3."):
            latest = (
                requested_version
                if requested_version.startswith("version_")
                else f"version_{normalize_version(requested_version)}"
            )
        ...
        bundle_url = f"{index_url.rsplit('/', 1)[0]}/{latest}/profile-bundle.json"
        bundle_response = requests.get(bundle_url, timeout=120)
        if bundle_response.status_code == 404:
            logger.info("No evaluated profile bundle published for %s", latest)
            return None
        ...
        with tempfile.TemporaryDirectory(prefix="prusa-profile-bundle-") as directory:
            ...
            if not isinstance(parser, PrusaSlicerParser):
                raise ParseError("evaluated profile bundles require PrusaSlicerParser")
            parsed = list(parser.parse_profile_bundle(bundle_path, profile_types))
            version = normalize_version(latest)
            self.reporter.update_status(
                f"Storing {len(parsed)} evaluated {slicer.value} profiles "
                f"(version {version})..."
            )
